function next_β(
    ...
    q_x_Σ_inv_x = zeros(data.K_fix,data.K_fix)
    q_x_Σ_inv_z = zeros(data.K_fix)
    for n in 1:data.N
        x = data.x_fix[:,:,n]
        q_x_Σ_inv = q[n] .* x * Σ_inv
        q_x_Σ_inv_x += q_x_Σ_inv * x'
        q_x_Σ_inv_z += q_x_Σ_inv * z_trans[:,n]
    end

    β_Σ_inv = q_x_Σ_inv_x + β_B0
    β_μ = β_Σ_inv \ (q_x_Σ_inv_z + β_B0 * β_μ0_trans)
...
function next_γ(
    ...
    for i in 1:data.n_ind
        q_x_Σ_inv_x = zeros(data.K_rnd,data.K_rnd)
        q_x_Σ_inv_z = zeros(data.K_rnd)
        ...
        γ_Σ_inv_i = q_x_Σ_inv_x + Ω_inv
        γ_Σ_inv[:,:,i] = γ_Σ_inv_i
        γ_μ[:,i] = γ_Σ_inv_i \ (q_x_Σ_inv_z + Ω_inv * η_trans)
    end
...
function mcmc_chain(
    chain_id::Int64, model::HierRobit, mcmc_args::McmcArgs, data::HierData,
...
function estimate(
    model::HierRobit, mcmc_args::McmcArgs, data::HierData,
    β_μ0::Vector{Float64}, β_B0::Symmetric{Float64,Array{Float64,2}},
    C0_inv::Symmetric{Float64,Array{Float64,2}},
    α0::Float64, β0::Float64
    )
    ...
    println(" ")
    println("Starting MCMC simulation for hierarchical robit model.")
    println(" ")
...
function predict_probs(
    ...
    Σ::Symmetric{Float64,Array{Float64,2}}, ν::Float64,
    ...
    for r in 1:n_sim
        ...
        v = v_fix .+ v_rnd
        for n in 1:data.N
            probs_sim[r,:,n] += robit_probs_fs(v[:,n], Σ_L, ν,
